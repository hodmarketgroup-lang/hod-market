...
import { getLogisticCaisse } from '../../services/api';
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer } from 'recharts';
import StatCard from '../../components/StatCard';

function fmt(m) { return Math.round(m || 0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' '); }

export default function LogisticCaisse() {
  const [data, setData] = useState({ journal: [], solde: 0 });

  useEffect(() => { getLogisticCaisse().then(r => setData(r.data)); }, []);
  ...
  return (
    <div>
      <h1 style={{ fontSize: 26, fontWeight: 700, marginBottom: '1.5rem', color: '#ff9800' }}>💰 HOD LOGISTIC - Caisse</h1>

      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: '2rem' }}>
        <StatCard label="Solde actuel" value={fmt(data.solde) + ' FCFA'} color="#ff9800" />
        <StatCard label="Total entrees" value={fmt(totalEntrees) + ' FCFA'} color="#00e676" />
      ...
      <div style={{ background: '#162436', borderRadius: 14, padding: '1.5rem', border: '1px solid rgba(255,255,255,0.07)', marginBottom: '2rem' }}>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <LineChart data={graphData}>
            ...
            <Line type="monotone" dataKey="solde" stroke="#ff9800" strokeWidth={2} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
